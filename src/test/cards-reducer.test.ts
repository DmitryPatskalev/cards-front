...
test('card should be removed', () => {
	 ...
	 const newState = {...startState, cards: startState.cards.filter(card => card._id !== '123456')}
	 const action = setCards(newState.cards)
	 const endState = cardsReducer(startState, action)
...
test('correct card should be updated', () => {
	 const newState = {
			...startState, cards: startState.cards.map(card => card._id === '123456' ?
				{...card, question: 'Who let the dogs out??', answer: 'It is me )))'} : card)
	 }

	 const action = setCards(newState.cards)
	 const endState = cardsReducer(startState, action)

	 expect(endState.cards[0].question).toBe('Who let the dogs out??')
	 expect(endState.cards[0].answer).toBe('It is me )))')
})